//公用的ip地址，公用的图片地址
  // ipUploadUrl = "http://192.168.28.251:8111/xddata/xdFileAllSysUpload.ashx?XKLX=CMW",
  // baseUrl = "/cmw",

  // httpUrl = "http://192.168.28.251:8111/xdData/xdDataManage.ashx?XKLX=CMW";
// var basePathImg = "http://192.168.28.251:8111/cmw/Widget";
...
//   baseUrl = "/cmw";
// }
var ipUrl = "http://192.168.28.251:8111/xdData/xdDataManage.ashx?XAction=GetDataInterface&XKLX=CMW", 
httpip = "http://192.168.28.251:8111"
...
function PostDataAsync(requestData, callback) { //同步的ajax请求
  var rv
  try {
    $.ajax({
      async: false,
...
function getAuth() {
  if (localStorage.getItem("adminSytoken")) {
    return localStorage.getItem("adminSytoken");
  } else {
...
function getdata(stattime, endtime) {
  PostIndexData({
    start_at: stattime,
    end_at: endtime
  }, function (returnData) {
    ...
    $.each(returnData.project_dt, function (index, data) {

      if (data.项目实施状态 == "") {
        $("#insideNumlib").text(data.数量)
      }
      if (data.项目实施状态 == "所有项目") {

        $("#sumNumlib").text(data.数量)
      }
      if (data.项目实施状态 == "未实施") {
        $("#inStorkeNumlib").text(data.数量)
      }
      if (data.项目实施状态 == "正在实施") {
        $("#outsideNumlib").text(data.数量)
      }
    })
...
//   // 饼状图等级
// function getDjdata(stattime, endtime){
//     PostIndexData({
//         start_at:stattime,
//          end_at:endtime
//      }, function(returnData) {
//         var newData = [];
//         for (let i = 0; i < returnData.category.length; i++) {
//             newData.push({ value: returnData.series[0].data[i], name: returnData.category[i] });
//         }
//         getchartD(newData);
//      })
